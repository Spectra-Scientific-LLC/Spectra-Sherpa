...
Enforces restrictions for HYBRID and ENTERPRISE mode deployments:
1. Rate limiting per user/IP (sliding window) - applies in HYBRID and ENTERPRISE modes
2. Global password protection (optional) - ENTERPRISE mode only
3. Session expiry enforcement - ENTERPRISE mode only
...
class EnterpriseEnforcementMiddleware(BaseHTTPMiddleware):
    """
    Enforces restrictions for Hybrid and Enterprise modes:
    1. Rate limiting per user/IP (sliding window) - HYBRID and ENTERPRISE modes
    2. Global password protection (if configured) - ENTERPRISE mode only
    3. Session expiry enforcement - ENTERPRISE mode only
    """

    # Paths that bypass enforcement restrictions
    PUBLIC_PATHS = {
        "/",
        "/api/v1/health",
        "/api/v1/config",
    ...
    async def dispatch(self, request: Request, call_next) -> Response:
        ...
        if is_enterprise():
            # 1. Enterprise Password Protection
            enterprise_pw = app_config.enterprise_password or app_config.demo_password
            if enterprise_pw:
                # Enforce on registration and initial access
                if path == "/api/v1/auth/register":
                    provided_pass = (
                        request.headers.get("X-Enterprise-Password")
                        or request.headers.get("X-Demo-Password")  # backward compat
                    )
                    if provided_pass != enterprise_pw:
                        return self._error_response(
                            status.HTTP_401_UNAUTHORIZED,
                            "Enterprise password required for registration",
                            {"header": "X-Enterprise-Password"}
                        )
        ...
        # === HYBRID + ENTERPRISE FEATURES ===
        # 3. Rate Limiting (applies in both Hybrid and Enterprise modes)
    ...
    def _check_session_expiry(self, token: str) -> Optional[Response]:
        """Check if session has expired based on enterprise expiry settings."""
        payload = decode_access_token(token)
        if not payload:
