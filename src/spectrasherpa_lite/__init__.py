"""SpectraSherpa Lite — local-first spectroscopy platform."""
...
class _AppAliasFinder(importlib.abc.MetaPathFinder):
    """Redirect ``import app.*`` to ``import spectrasherpa_lite.app.*``.
    ...
    def _should_activate(cls) -> bool:
        ...
        # spectrasherpa_lite/app on sys.path).  Any other ``app`` module
    ...
    def find_spec(self, fullname: str, path=None, target=None):
        ...
        if fullname == "app" or fullname.startswith("app."):
            real = "spectrasherpa_lite." + fullname
            try:
