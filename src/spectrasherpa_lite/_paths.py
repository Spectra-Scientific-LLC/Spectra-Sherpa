"""Dual-mode path resolution for SpectraSherpa Lite.
...
When pip-installed, user data goes to ``~/.spectrasherpa/``.
"""
...
# The directory that contains *this* file (spectrasherpa_lite/)
...
def get_package_root() -> Path:
    """Return the ``spectrasherpa_lite/`` directory (always valid)."""
    return _PACKAGE_DIR
...
def get_default_data_dir() -> Path:
    ...
    if root is not None:
        return root / "data"
    return Path.home() / ".spectrasherpa"
