...
class Node(ABC):
    ...
    async def run(self, *inputs: Any, **kwargs: Any) -> Any:
        ...
        Execution is offloaded to a thread pool so CPU-bound scientific
        computations (PCA, curve_fit, sklearn, etc.) don't block the
        async event loop.  Nodes that perform async I/O internally
        (e.g. database queries) work transparently because each thread
        gets its own event loop via ``asyncio.run()``.
        ...
        try:
            ...
            coro = self.execute(**kwargs) if kwargs else self.execute(*inputs)
            self.result = await loop.run_in_executor(None, asyncio.run, coro)
