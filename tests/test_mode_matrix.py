...
class TestIsEgressEnabled:
    ...
    def test_hybrid_egress_enabled_when_healthy(self):
        ...
        with patch("app.core.security.app_config", cfg), \
             patch("app.services.network_health.get_network_health_service", return_value=mock_health):
            from app.core.security import is_egress_enabled
...
class TestExportAllowed:
    ...
    async def test_local_always_allows_export(self):
        ...
        with patch("app.core.mode_policy.app_config", _make_config(mode="local")):
...
class TestTokenTTL:
    ...
    def test_local_token_ttl_is_long(self):
        ...
        with patch.dict("os.environ", {"APP_MODE": "local"}, clear=False):
            from app.core.config import _get_int
...
class TestMCPToolSystem:
    ...
    async def test_tool_egress_blocked_when_disabled(self):
        ...
        from app.services.tools import tool_registry
        from app.services.tools.schemas import ToolDefinition, ToolOrigin
        ...
        try:
            ...
            # Patch at source — executor lazy-imports from app.core.security
            with patch("app.core.security.is_egress_enabled", return_value=False):
