...
class TestAllNodeTypeRefs:
            ...
            if meta.input_ports:
                for port in meta.input_ports:
                    try:
                        ...
                        errors.append(
                            f"{meta.node_type} input '{port.name}': {e}"
                        )

        if errors:
            pytest.fail(
                f"Unresolvable type_refs on input ports:\n"
            ...
            if meta.output_ports:
                for port in meta.output_ports:
                    try:
                        ...
                        errors.append(
                            f"{meta.node_type} output '{port.name}': {e}"
                        )

        if errors:
            pytest.fail(
                f"Unresolvable type_refs on output ports:\n"
                + "\n".join(f"  {e}" for e in errors)
            )
            ...
            for port in (meta.input_ports or []) + (meta.output_ports or []):
                try:
                    ...
                    if td.category not in valid_categories:
                        errors.append(
                            f"{meta.node_type} port '{port.name}': "
                            f"category '{td.category}' not in known categories"
                        )
                except (KeyError, ValueError) as e:
                    errors.append(
                        f"{meta.node_type} port '{port.name}': {e}"
                    )

        if errors:
            pytest.fail(
                f"Invalid port categories:\n"
                + "\n".join(f"  {e}" for e in errors)
            )
    ...
    def test_no_port_type_equals_in_declarations(self):
        ...
        import ast
        nodes_dir = Path(__file__).resolve().parent.parent / "src" / "spectra_sherpa" / "app" / "services" / "dag" / "nodes"
        ...
        if violations:
            pytest.fail(
                f"Found port_type= keyword args (should be type_ref=):\n"
    ...
    def test_validate_node_meta_utility_per_node(self):
        ...
        if errors:
            pytest.fail(
                "validate_node_meta reported errors:\n"
                + "\n".join(f"  {e}" for e in errors)
            )
