...
Docker-capable VPS) in enterprise mode.
...
-   Review `docs/REQUIREMENTS.md` for required repo layout and version baselines.
...
Create a `.env` file in `/opt/spectra-platform/spectra-ops/docker/`:

```bash
# /opt/spectra-platform/spectra-ops/docker/.env
...
# === REQUIRED ===
# IMPORTANT: Set explicitly — auto-generated key changes on restart, losing stored API keys
MASTER_ENCRYPTION_KEY=<paste your generated encryption key>
# Enterprise mode requires PostgreSQL (SQLite will be rejected at startup)
...
# SITE_PROFILE=demo               # Marketing label: demo, production, or internal
...
```bash
cd /opt/spectra-platform/spectra-ops/docker
...
`DATABASE_URL` points to SQLite. SQLite is only acceptable for local/hybrid
...
|----------|----------|---------|-------------|
| `APP_MODE` | Yes | `local` | Set to `enterprise` for DO deployment |
...
| `MASTER_ENCRYPTION_KEY` | Required* | Auto-generated | Stable key for API key encryption. *Data loss on restart if unset. |
| `DATABASE_URL` | Yes (enterprise) | — | PostgreSQL connection string (enterprise hard-fails with SQLite) |
...
| `ENTERPRISE_PASSWORD` | No | (none) | Registration gate password (`DEMO_PASSWORD` also accepted) |
| `SITE_PROFILE` | No | `demo` | Marketing label: `demo`, `production`, or `internal` |
