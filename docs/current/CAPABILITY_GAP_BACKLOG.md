...
- [x] Demo graceful degradation/offline behavior does not match matrix claim.
  ...
  - Follow-up: either implement demo degradation path or revise docs to hybrid-only.
  - Resolution (2026-02-08): **Closed via documentation alignment**. Matrix now states demo is `Rate-limited cloud (no auto-fallback)` in `docs/current/CURRENT_CAPABILITIES.md:22`.

## Priority 2 (High)

- [x] Demo "tiered egress + audit" claim is not aligned with current remote audit implementation.
  ...
  - Follow-up: decide whether demo should support remote audit. Implement or update matrix wording.
  - Resolution (2026-02-08): **Closed via documentation alignment**. Matrix now differentiates `Hybrid: tiered egress + remote audit` vs `Demo: tiered egress (local audit only)` at `docs/current/CURRENT_CAPABILITIES.md:21`.

- [x] "Implemented and tested across local/hybrid/demo" is overstated vs current visible test coverage.
  ...
  - Resolution (2026-02-08): **Closed via documentation alignment**. Intro and `Testing Status` section now explicitly describe limited hybrid/demo automated coverage (`docs/current/CURRENT_CAPABILITIES.md:3`, `docs/current/CURRENT_CAPABILITIES.md:157`).
...
- [ ] Rate-limit response header behavior is inconsistent with doc wording.
  ...
  - Code refs: `src/spectra_sherpa/app/core/demo_enforcement.py:116`, `src/spectra_sherpa/app/core/demo_enforcement.py:117`, `src/spectra_sherpa/app/api/v1/routes/llm.py:42`, `src/spectra_sherpa/app/api/v1/routes/nist.py:44`
...
2. Add test coverage proving local/hybrid/demo behavior for matrix-listed features.
